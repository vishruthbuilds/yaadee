...
const TimeCapsule = () => {
  ...
  const [currentIndex, setCurrentIndex] = useState(0); // Index of the LEFT image of the current spread
  ...
  const turnPage = () => {
    ...
    if (currentIndex + 2 >= bookImages.length) {
       setSection('reel');
       return;
    }
    setIsFlipping(true);
  ...
  // True Spread Logic
  const leftImg = bookImages[currentIndex];
  const rightImg = bookImages[currentIndex + 1];
  const nextLeft = bookImages[currentIndex + 2];
  const nextRight = bookImages[currentIndex + 3];
  ...
  return (
    <div className="min-h-screen bg-[#fdfbf7] overflow-hidden selection:bg-accent/20">
      <AnimatePresence mode="wait">
            ...
            <div className="relative perspective-2000 w-full max-w-5xl aspect-[1.414/1] flex items-center justify-center">
              <div className="relative w-full h-full flex bg-[#f5f2eb] rounded shadow-2xl overflow-hidden border border-stone-200/50">
                ...
                <div className="w-1/2 h-full bg-[#faf7f2] relative overflow-hidden border-r border-stone-200/30">
                  ...
                  {leftImg && <img src={leftImg} className="w-full h-full object-cover" alt="Left" />}
                </div>

                {/* STATIC BASE RIGHT (Next Right) */}
                <div className="w-1/2 h-full bg-[#faf7f2] relative overflow-hidden">
                  <div className="absolute inset-0 page-texture"></div>
                  {nextRight && <img src={nextRight} className="w-full h-full object-cover" alt="Next Right" />}
                </div>
                ...
                >
                   ...
                   <div className="absolute inset-0 backface-hidden bg-[#faf7f2] shadow-xl overflow-hidden border-l border-stone-100">
                      <div className="absolute inset-0 page-texture"></div>
                      {rightImg && <img src={rightImg} className="w-full h-full object-cover" alt="Front" />}
                   </div>
                   {/* Back Side: Next Left Page (New set of images) */}
        ...
        {/* ... Reel and Video sections stay the same ... */}
        {section === 'reel' && (
          <motion.section key="reel" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-screen flex flex-col items-center justify-center bg-[#0d0d0c] relative overflow-hidden">
            ...
            <div className="w-full relative py-12 flex flex-col items-center">
              <div ref={reelRef} className="w-full flex gap-8 px-[25%] overflow-x-hidden">
              ...
              <div className="mt-24 relative flex flex-col items-center">
                ...
                <div ref={crankRef} className="relative w-48 h-48 flex items-center justify-center">
                  ...
                  <motion.div 
                    drag dragConstraints={{ left: 0, right: 0, top: 0, bottom: 0 }} dragElastic={0} onDrag={handleCrankDrag} onDragEnd={() => { lastAngle.current = null; }}
                  ...
                  >
                    <svg width="100%" height="100%" viewBox="0 0 100 100" className="drop-shadow-2xl">
                       ...
                       <defs>
                         <linearGradient id="brass-grad" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#b8860b" /><stop offset="50%" stopColor="#ffd700" /><stop offset="100%" stopColor="#b8860b" /></linearGradient>
                         <linearGradient id="knob-grad" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="#4a4a4a" /><stop offset="100%" stopColor="#1a1a1a" /></linearGradient>
                       </defs>
                    </svg>
                  </motion.div>
                  <div className="w-10 h-10 rounded-full bg-stone-800 border-4 border-stone-700 shadow-xl z-30 pointer-events-none" />
                </div>
                <button onClick={() => setSection('video')} className="mt-16 btn-primary border-white text-white hover:bg-white hover:text-black px-12">Watch Final Movie &rarr;</button>
              </div>
            </div>
          </motion.section>
        )}
        ...
        {section === 'video' && (
          <motion.section key="video" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-screen bg-black flex items-center justify-center">
            {finalVideo ? <video src={finalVideo} autoPlay loop className="w-full h-full object-cover opacity-80" /> : <div className="text-center p-8 text-stone-700 font-serif italic text-2xl">Finishing the reel...</div>}
          </motion.section>
        )}
      </AnimatePresence>
    </div>
